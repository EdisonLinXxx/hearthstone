...
- battle 阶段已增加异常优先采样：当 mana / cost OCR 拒识、不可信、规则校验失败、`ocr_wait_mana` / `ocr_wait_cost`、`hand_cards_not_ready`、legacy debug candidates 与 final cards 明显不一致等情况出现时，会额外保存 `ocr_anomaly` 样本，并落 `meta.json`
