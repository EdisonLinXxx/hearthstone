...
- 当前手牌检测不再只依赖绿色高亮，新增了底部约束、左下角黑名单过滤，以及 `card_score` / `playable_score` 双层门槛
- 拖拽起点改成卡牌下半部固定锚点，避免把非手牌装饰区或空白区当作出牌起点
- `1440x900` 的 `hand` 区域和 `hand_detection` 阈值已进一步收紧，`1600x900` 也补齐了新字段

结束回合：

- 出牌后新增结束回合冷却，当前默认 `3.0s`
- 结束回合前需要连续多帧确认按钮稳定可点，避免动画尚未结束时抢点失败

结果页归一化：

- `runtime.py` 已增加菜单/排队页排斥条件
- 当页面更像 `main_menu` / `battle_menu` / `queue_page` / `matching` 时，不再把 `unknown` 误升格为 `result_continue`
- 进入上述菜单类场景时会清理最近战斗上下文

OCR 方向：

- 当前决定先做 OCR，而不是继续死磕现有手牌可出牌规则
- 已内置自动 OCR 采样：在点击 `结束回合` 前自动保存 `full`、`mana`、`hand` 样本到 `bot/samples/<profile>/ocr_auto_turn_end/`
- 自动采样可由启动参数控制：
  - `--ocr-auto-sample`
  - `--no-ocr-auto-sample`
...
1. 持续跑 `1440x900`，积累 OCR 自动样本
2. 优先恢复 mana / 手牌费用 OCR，先解决“法力不足仍然拖牌”
3. OCR 跑稳后再决定是否继续引入 YOLO 做手牌和按钮检测
